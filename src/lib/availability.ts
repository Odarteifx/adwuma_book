...
/**
 * Returns YYYY-MM-DD dates that have at least one available slot in the next N days.
 * Used by AI chat to suggest bookable dates.
 */
export async function getAvailableDates(
  ...
  const blockedSet = new Set((blockedRows || []).map((b: { date: string }) => b.date));
  ...
  while (current <= endDate) {
    ...
    for (let m = startMinutes; m + durationMinutes <= endMinutes; m += interval) {
      ...
      if (inBreak) continue;
      const overlappingCount = existingBookings.filter((bk) => {
    ...
    if (hasAvailable) available.push(dateStr);
    current.setDate(current.getDate() + 1);
  }
